...
const onetimeCodeSigninRequest = async (headers, body) => {
  ...
  console.log(
    "dynamo",
    result.Item.OnetimeCodeId.S,
    result.Item.Code.S,
    result.Item.OnetimeCodeExpireDate.N,
    result.Item.OnetimeCodeStatus.S
  );
...
const onetimeCodeSignin = async (headers, body) => {
  ...
  await ddb
    .updateItem({
      ...
      UpdateExpression:
        "set OnetimeCodeStatus=:s, IdToken=:idToken, AccessToken=:accessToken",
