...
exports.authorize = async (event, context, callback) => {
  ...
  if (
    ...
    console.error(
      "missing MeetingId, AttendeeId, JoinToken parameters",
  ...
  //// (3) check joinToken
  if (
    attendeeInfo.Attendee.JoinToken !== event.queryStringParameters.joinToken
  ) {
    console.error(
      `failed to authenticate with join token ${attendeeInfo.Attendee.JoinToken} - ${event.queryStringParameters.joinToken}`
    );
    return generatePolicy("me", "Deny", event.methodArn, {});
  }
