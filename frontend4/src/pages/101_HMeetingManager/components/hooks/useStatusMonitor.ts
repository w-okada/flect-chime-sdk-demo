...
export const useStatusMonitor = () => {
    ...
    useEffect(() => {
        if (!chimeClient?.attendeeId) {
            // not yet ready
            return;
        }
        //// exclude hmm and shared contents
        let meetingActive = true;
        const pureAttendees = Object.keys(chimeClient!.attendees).filter((x) => {
            return x.indexOf(chimeClient!.attendeeId!) < 0;
        });
        ...
        const attendeeList = pureAttendees.reduce((prev, cur) => {
            return `${prev}, ${cur}(${chimeClient.getUserNameByAttendeeIdFromList(cur)})`;
        }, "");
