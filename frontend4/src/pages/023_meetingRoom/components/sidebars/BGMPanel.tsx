...
export const BGMPanel = () => {
    ...
    const playBGM = (path: string) => {
        ...
        elem.onloadeddata = async (e) => {
            ...
            chimeClient!.audioInputDeviceSetting?.setBackgroundMusic(mediaStream);
            elem.play();
            setIsPlaying(true);
        };
    };
    ...
    const handleVolumeChange = (event: any, newValue: number | number[]) => {
        ...
        if (typeof newValue === "number") {
            chimeClient!.audioInputDeviceSetting?.setBackgroundMusicVolume(newValue);
            elem.volume = newValue;
            setVolume(newValue);
        } else if (Array.isArray(newValue)) {
            chimeClient!.audioInputDeviceSetting?.setBackgroundMusicVolume(newValue[0]);
    ...
    return (
        <div className={classes.root}>
            <div className={classes.seList}>{ses}</div>
