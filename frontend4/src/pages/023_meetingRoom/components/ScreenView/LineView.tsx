...
export const LineView = ({ excludeSpeaker, excludeSharedContent, width, height, viewInLine }: Props) => {
    ...
    const targetNames = targetTiles.reduce<string>((names, cur) => {
        return `${names}_${chimeClient!.getUserNameByAttendeeIdFromList(cur.boundAttendeeId!)}`;
    }, "");
    const view = useMemo(() => {
                ...
                {targetTiles?.map((tile, i) => {
                    ...
                    return (
                        <div key={`line-view-${i}`} style={{ height: height - 2, margin: "1px", flex: "0 0 auto", position: "relative" }}>
                            ...
                            <div style={{ position: "absolute", lineHeight: 1, fontSize: 14, height: 15, top: height - 20, left: 5, background: tile.boundAttendeeId === chimeClient!.activeSpeakerId ? "#ee7777cc" : "#777777cc" }}>{chimeClient!.getUserNameByAttendeeIdFromList(tile.boundAttendeeId ? tile.boundAttendeeId : "")}</div>
                        </div>
                    );
                })}
            </div>
