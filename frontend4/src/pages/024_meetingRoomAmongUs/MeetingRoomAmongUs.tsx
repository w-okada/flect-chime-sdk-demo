...
export const MeetingRoomAmongUs = () => {
    ...
    const targetTilesId = Object.keys(chimeClient!.videoTileStates).reduce<string>((sum, cur) => {
        return `${sum}-${cur}`;
    }, "");
    ...
    const mainScreen = useMemo(() => {
        if (chimeState.arenaViewScreen && amongusGameState?.state != 2) {
            ...
            if (viewMode === "MultiTileView") {
                return <video id="tileView" style={{ width: "97%", height: "100%", borderStyle: "solid", borderColor: blueGrey[900] }} />;
            } else {
                const videoTileNum =
                    chimeClient!.meetingSession?.audioVideo.getAllRemoteVideoTiles().filter((x) => {
                        return x.state().isContent === false;
                    }).length || 0;
                ...
                for (let i = 0; i < rows; i++) {
                    ...
                    for (let j = 0; j < cols; j++) {
                        ...
                        cells.push(<video id={`userView${index}`} style={{ width: `${100 / cols - 2}%`, borderStyle: "solid", borderColor: blueGrey[900] }} />);
                    }
                    const row_cell = <div style={{ display: "flex", flexDirection: "row" }}>{cells}</div>;
            ...
            // show map
            const player = amongusGameState?.players.find((x) => {
                return x.attendeeId === chimeClient!.attendeeId;
            });
    ...
    useEffect(() => {
        ...
        return () => {
            videoEl.removeEventListener("click", listenEvent);
        };
    }, []); // eslint-disable-line
    ...
    const startHmmClientClicked = async () => {
        ...
        const elecCmd = `docker run --rm -e CODE=${res.code} -e UUID=${res.uuid} -e MEETING_NAME=${res.meetingName} -e ATTENDEE_ID=${res.attendeeId} -e RESTAPI_ENDPOINT=${res.restAPIEndpoint} -e BUCKET_ARN=${res.bucketArn} -e BUCKET_NAME=${res.bucketName} -ti -p 3000:3000 -v \`pwd\`:/work dannadori/hmm`;
    ...
    const managerStateComp = useMemo(() => {
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === false && (!chimeClient!.hmmClient!.hmmLastStatus || chimeClient!.hmmClient!.hmmLastStatus === "N/A")) {
            // Not active
            return (
                <>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === false && chimeClient!.hmmClient!.hmmLastStatus === "PROVISIONING") {
            // Invoking && Provisioning
            return (
                <>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === true && chimeClient!.hmmClient!.hmmLastStatus === "PROVISIONING") {
            // Invoking && Provisioning
            return (
                <>
                    <Tooltip title={"invoking hmm: provisioning"}>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === false && chimeClient!.hmmClient!.hmmLastStatus === "PENDING") {
            // Invoking && PENDING
            return (
                <>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === true && chimeClient!.hmmClient!.hmmLastStatus === "PENDING") {
            // already invoked (1)
            return (
                <>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === false && chimeClient!.hmmClient!.hmmLastStatus === "RUNNING") {
            // already invoked (2)
            return (
                <>
        ...
        } else if (chimeClient!.hmmClient!.hmmActive === true && chimeClient!.hmmClient!.hmmLastStatus === "RUNNING") {
            //invoked
            return (
                <>
                    <Tooltip title={"hmm active"}>
                        <IconButton
                            ...
                            onClick={() => {
                                chimeClient!.hmmClient!.sendTerminate();
                            }}
                        >
    ...
    const recordingStateComp = useMemo(() => {
        return chimeClient!.hmmClient!.hmmRecording ? (
            <>
                <Tooltip title={"recording"}>
                    <IconButton
                        classes={{ root: classes.menuButton }}
                        onClick={() => {
                            chimeClient!.hmmClient!.sendStopRecord();
                        }}
                    >
            ...
            <>
                <Tooltip title={"not recording"}>
                    <IconButton
                        classes={{ root: classes.menuButton }}
                        onClick={() => {
                            chimeClient!.hmmClient!.sendStartRecord();
                        }}
                    >
    ...
    const shareTileViewStateComp = useMemo(() => {
        return chimeClient!.hmmClient!.hmmShareTileview ? (
            <>
                <Tooltip title={"share tile view"}>
                    <IconButton
                        classes={{ root: classes.menuButton }}
                        onClick={() => {
                            chimeClient!.hmmClient!.sendStopShareTileView();
                        }}
                    >
            ...
            <>
                <Tooltip title={"not share share tile view"}>
                    <IconButton
                        classes={{ root: classes.menuButton }}
                        onClick={() => {
                            chimeClient!.hmmClient!.sendStartShareTileView();
                        }}
                    >
    ...
    const arenaMicrophoneComp = useMemo(() => {
        return chimeState.arenaMicrophone ? (
            <>
    ...
    const arenaSpeakerComp = useMemo(() => {
        return chimeState.arenaSpeaker ? (
            <>
    ...
    const arenaShareScreenComp = useMemo(() => {
        return chimeState.arenaShareScreen ? (
            <>
    ...
    const arenaViewScreenComp = useMemo(() => {
        return chimeState.arenaViewScreen ? (
            <>
    ...
    const gameStateComp = useMemo(() => {
        return (
            <div>
                <div>{amongusGameState ? STATES[amongusGameState.state] : "N/A"}</div>
                <div>
    ...
    const userState = useMemo(() => {
        return <div>State:{chimeState.name}</div>;
    }, [chimeState]);
        ...
        return (
            <div>
                {amongusGameState?.players
                    ...
                    .map((x) => {
                        return (
                            <div key={`attendee_${x.attendeeId}`}>
                                {(x.isDead && x.isDeadDiscovered) || x.disconnected ? (
                                    <div>
                                        <img style={{ width: "20%" }} src={ICONS_DEAD[x.color]} alt="dead"></img>
                                        {x.name}/{x.chimeName}
                                    </div>
                                ) : (
                                    <div>
                                        <img style={{ width: "20%" }} src={ICONS_ALIVE[x.color]} alt="alive"></img>
                                        {x.name}/{x.chimeName}
                                    </div>
                                )}
                            </div>
        ...
        <div className={classes.root}>
            <div id="header" style={{ display: "flex", flexDirection: "row" }}>
                <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <div>{chimeClient?.hmmClient?.hmmPublicIp ? `http://${chimeClient?.hmmClient?.hmmPublicIp}:3000` : ""}</div>
                    <div>lastupdate:{stateLastUpdateTime}</div>
                </div>
            ...
            <div id="main" style={{ height: "100%", display: "flex", flexDirection: "row" }}>
                ...
                <div style={{ width: "65%", height: "100%", display: "flex", flexDirection: "column" }}>
                    <div style={{ height: "20%", display: "flex", flexDirection: "row" }}>
                        <div style={{ width: "25%", height: "100%", textAlign: "right" }}>click to share your screen</div>
                        ...
                        <div style={{ width: "30%", height: "100%", alignItems: "center" }}></div>
                    </div>
            ...
            <SettingDialog open={settingDialogOpen} onClose={() => setSettingDialogOpen(false)} viewMode={viewMode} setViewMode={setViewMode} debugEnable={debugEnable} setDebugEnable={setDebugEnable} screenSize={screenSize} setScreenSize={setScreenSize} />
            <div id="info1" />
        </div>
    );
};
