...
export const REGIONS = ["North America", "Asia", "Europe"];
...
export const COLORS = ["red", "blue", "green", "pink", "orange", "yellow", "black", "white", "purple", "brown", "cyan", "lime", "maroon", "rose", "banana", "tan", "sunset", "sunset", "sunset"];
...
export class RealtimeSubscribeHMMModuleAmongUsServer {
    ...
    updateGameState = (ev: string, data: string) => {
        ...
        this._gameState.players.forEach((x) => {
            if (x.attendeeId) {
                const newPlayer = newGameState.players.find((y) => {
    ...
    registerUserName = (userName: string, attendeeId: string, chimeUserName: string) => {
        const unregisterTarget = this._gameState.players.find((x) => {
        ...
        const targetPlayer = this._gameState.players.find((x) => {
