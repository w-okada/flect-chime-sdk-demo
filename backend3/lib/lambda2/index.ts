...
const Resources = {
    Root: "/",
    Meetings: "/meetings",
    Meeting: "/meetings/{exMeetingId}",
    Attendees: "/meetings/{exMeetingId}/attendees",
    Attendee: "/meetings/{exMeetingId}/attendees/{attendeeId}",
    Operation: "/meetings/{exMeetingId}/attendees/{attendeeId}/operations/{operation}",
...
export const handler = async (event: any, context: any, callback: any) => {
    ...
    switch (resource) {
        case Resources.Root:
            await handleRoot(method, pathParams, body, callback, userInfo);
            break;
        case Resources.Meetings:
            await handleMeetings(method, pathParams, body, callback, userInfo);
            break;
...
// (0) Root
const handleRoot = async (method: string, pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
...
// (1) meetings
const handleMeetings = async (method: string, pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
    console.log(`HANDLE Meetings: ${method} ${body}`);
    switch (method) {
        case Methods.POST:
            await handlePostMeetings(pathParams, body, callback, userInfo);
            break;
        case Methods.GET:
            await handleGetMeetings(pathParams, body, callback, userInfo);
            break;
        default:
...
//// (1-1) post meetings
const handlePostMeetings = async (pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
...
//// (1-2) list meetings
const handleGetMeetings = async (pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
...
const handleGetMeeting = async (accessToken: string, pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
    let res: HTTPResponseBody;
    const exMeetingId = pathParams["exMeetingId"];

    if (!exMeetingId) {
        console.log(`parameter error: ${exMeetingId}`);
        res = {
            success: false,
        ...
        if (!result) {
            ...
            const httpRes: HTTPGetMeetingInfoResponse = result;
            res = {
                success: true,
                code: Codes.SUCCESS,
                data: httpRes,
...
const handleGetAttendee = async (accessToken: string, pathParams: { [key: string]: string }, body: any, callback: any, userInfo: UserInfoFromCognito) => {
    ...
    if (!exMeetingId || !attendeeId) {
        console.log(`parameter error: ${exMeetingId}, ${attendeeId}`);
        res = {
            success: false,
            code: Codes.PARAMETER_ERROR,
        };
